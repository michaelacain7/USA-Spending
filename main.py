...
class USASpendingMonitor:
    ...
    Only alerts when:
    1. The recipient matches a publicly traded company (via SEC EDGAR tickers list)
    2. The contract amount is > 1% of the company's market cap (materiality filter)
    
    Includes stock ticker symbol in all alerts.
    """
    
    # Common suffixes/words to strip when matching company names
    STRIP_SUFFIXES = [
        ', INC.', ', INC', ' INC.', ' INC', ', LLC', ' LLC',
        ', L.L.C.', ', CORP.', ', CORP', ' CORP.', ' CORP',
        ', CO.', ', CO', ' CO.', ', LTD.', ', LTD', ' LTD.',
        ' LTD', ', L.P.', ' L.P.', ', LP', ' LP',
        ', INCORPORATED', ' INCORPORATED', ', CORPORATION', ' CORPORATION',
        ', COMPANY', ' COMPANY', ', GROUP', ' HOLDINGS',
    ...
    def __init__(self):
        ...
        self.tickers_cache_file = DATA_DIR / "sec_tickers.json"
        self.marketcap_cache_file = DATA_DIR / "marketcap_cache.json"
        self.seen_awards: Set[str] = set()
        self.running = False
        self.thread = None
        
        # SEC tickers: maps cleaned company name -> {ticker, full_name}
        self.company_tickers: Dict[str, Dict] = {}
        # Market cap cache: ticker -> {market_cap, timestamp}
        self.marketcap_cache: Dict[str, Dict] = {}
    ...
    def _load_marketcap_cache(self):
        if self.marketcap_cache_file.exists():
            try:
                with open(self.marketcap_cache_file) as f:
                    self.marketcap_cache = json.load(f)
            except Exception:
                self.marketcap_cache = {}
    
    def _save_marketcap_cache(self):
        try:
            with open(self.marketcap_cache_file, 'w') as f:
                json.dump(self.marketcap_cache, f)
        except Exception:
            pass
    ...
    def _clean_name(name: str) -> str:
        ...
        for suffix in USASpendingMonitor.STRIP_SUFFIXES:
    ...
    def _load_sec_tickers(self):
        """Download SEC EDGAR company tickers list and build lookup dict.
        Caches locally for 7 days."""
        
        # Check cache
        if self.tickers_cache_file.exists():
            try:
                age = time.time() - self.tickers_cache_file.stat().st_mtime
                if age < 604800:  # 7 days
                    with open(self.tickers_cache_file) as f:
                        self.company_tickers = json.load(f)
                    self.logger.info(f"Loaded {len(self.company_tickers)} company tickers from cache")
                    return
            except Exception:
                pass
        
        try:
            self.logger.info("Downloading SEC EDGAR company tickers...")
            resp = self.session.get(
                'https://www.sec.gov/files/company_tickers.json',
                headers={'User-Agent': 'GovMonitor contact@example.com'},
                timeout=30
            )
            resp.raise_for_status()
            raw = resp.json()
            ...
            for entry in raw.values():
                ...
                # Store under cleaned name
                cleaned = self._clean_name(title)
                if cleaned:
                    self.company_tickers[cleaned] = {
                        'ticker': ticker,
                        'full_name': title,
                    }
            ...
            with open(self.tickers_cache_file, 'w') as f:
                json.dump(self.company_tickers, f)
            
            self.logger.info(f"Loaded {len(self.company_tickers)} company tickers from SEC EDGAR")
            
        except Exception as e:
            self.logger.error(f"Failed to load SEC tickers: {e}")
            # Try stale cache
            if self.tickers_cache_file.exists():
                try:
                    with open(self.tickers_cache_file) as f:
    ...
    def _match_to_ticker(self, recipient_name: str) -> Optional[Dict]:
        """Try to match a USASpending recipient name to a public company ticker.
        
        Returns: {ticker, full_name, match_score} or None
        """
        if not self.company_tickers:
            return None
        
        cleaned = self._clean_name(recipient_name)
        if not cleaned or len(cleaned) < 3:
            return None
        
        # 1. Exact match on cleaned name
        if cleaned in self.company_tickers:
            info = self.company_tickers[cleaned]
            return {
                'ticker': info['ticker'],
        ...
        # 2. Check if recipient name starts with or contains a known company name
        #    (handles "GENERAL DYNAMICS INFORMATION TECHNOLOGY, INC." -> "GENERAL DYNAMICS")
        best_match = None
        best_score = 0
        
        for known_name, info in self.company_tickers.items():
            # Skip very short names (avoid false matches on "A", "AT" etc.)
            if len(known_name) < 4:
                continue
            
            # Check if the cleaned recipient starts with the known company name
            if cleaned.startswith(known_name):
                score = len(known_name)  # Longer prefix = better match
                if score > best_score:
                    best_score = score
                    best_match = {
                        'ticker': info['ticker'],
            ...
            # Check if the known company starts with the recipient
            elif known_name.startswith(cleaned):
                score = len(cleaned)
                if score > best_score and score >= 8:  # Need decent length
                    best_score = score
                    best_match = {
                        'ticker': info['ticker'],
                        'full_name': info['full_name'],
        ...
        if best_match and best_match['match_score'] >= 90:
            return best_match
        
        # 3. Fuzzy match using rapidfuzz (only if exact/prefix didn't work)
        all_names = list(self.company_tickers.keys())
        results = process.extract(cleaned, all_names, scorer=fuzz.token_sort_ratio, limit=3)
        
        if results:
            top_name, top_score, _ = results[0]
            if top_score >= 85:
                info = self.company_tickers[top_name]
                return {
                    'ticker': info['ticker'],
                    'full_name': info['full_name'],
    ...
    def _get_market_cap(self, ticker: str) -> Optional[float]:
        """Get market cap for a ticker using yfinance. Caches for 24 hours."""
        
        # Check cache (valid for 24 hours)
        if ticker in self.marketcap_cache:
            cached = self.marketcap_cache[ticker]
        ...
        try:
            ...
            if market_cap and market_cap > 0:
                self.marketcap_cache[ticker] = {
                    'market_cap': market_cap,
                    'timestamp': time.time(),
                }
                self._save_marketcap_cache()
                self.logger.debug(f"Market cap for {ticker}: ${market_cap:,.0f}")
                return market_cap
            
        except Exception as e:
            self.logger.debug(f"Could not get market cap for {ticker}: {e}")
        
        return None
    
    def check_awards(self):
        """Check for new contract awards to publicly traded companies."""
        try:
            min_materiality = float(os.environ.get('MIN_MATERIALITY_PERCENT', '1.0'))
            ...
            for award in results:
                ...
                # === FILTER 1: Must match a publicly traded company ===
                match = self._match_to_ticker(recipient)
                if not match:
                    continue
                
                ticker = match['ticker']
                
                # === FILTER 2: Must be > 1% of market cap ===
                market_cap = self._get_market_cap(ticker)
                if market_cap is None:
                    self.logger.debug(f"Skipping {ticker} ({recipient}) — no market cap data")
                    continue
                
                materiality = (amount / market_cap) * 100
                if materiality < min_materiality:
                    self.logger.debug(
                        f"Skipping {ticker}: ${amount:,.0f} is {materiality:.2f}% of "
                        f"${market_cap:,.0f} market cap (below {min_materiality}% threshold)"
                    )
                    continue
                ...
                embed = {
                    ...
                    "footer": {"text": f"USASpending.gov Monitor | Match: {match['match_score']}%"}
                }
                
                send_discord_webhook(embed, self.logger)
                self.logger.info(
                    f"ALERT: {ticker} — ${amount:,.0f} contract "
                    f"({materiality_str} of ${market_cap:,.0f} mcap) from {agency}"
                )
                time.sleep(1)
            
            if new_count > 0:
                self._save_seen()
                self.logger.info(f"Processed {new_count} new awards, alerted on {alerted_count}")
