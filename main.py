...
class Config:
    ...
    # Debug mode
    debug_mode: bool = True  # Enable verbose logging to debug API issues
    ...
    def _fetch_award_notices(self, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            if self.config.sam_api_key:
                ...
                self.logger.debug(f"  Using SAM.gov API key: {self.config.sam_api_key[:10]}...")
            else:
                self.logger.debug(f"  WARNING: No SAM.gov API key configured!")
            
            self.logger.debug(f"  SAM.gov API request: {url}")
            self.logger.debug(f"  SAM.gov API params: postedFrom={params.get('postedFrom')}, postedTo={params.get('postedTo')}, ptype={params.get('ptype')}")
            ...
            if response.status_code == 200:
                ...
                self.logger.debug(f"SAM.gov API response keys: {list(data.keys())}")
                self.logger.debug(f"SAM.gov API totalRecords: {data.get('totalRecords', 'N/A')}")
                ...
                # Log first opportunity structure for debugging
                if opportunities and len(opportunities) > 0:
                    self.logger.debug(f"SAM.gov first opp keys: {list(opportunities[0].keys())[:10]}")
                ...
                for opp in opportunities:
                    ...
                    if award and award.get('recipient_name'):
                        ...
                        self.logger.debug(f"SAM.gov skipped opp (no recipient): {opp.get('title', 'No title')[:50]}")
            else:
                self.logger.debug(f"SAM.gov Opportunities API returned {response.status_code}: {response.text[:500]}")
