...
class PublicCompanyDatabase:
    ...
    def _add_auto_variations(self, ticker: str, name: str):
        ...
        if len(words) >= 2:
            ...
            # First word only if it's distinctive (>4 chars)
            if len(words[0]) > 4 and words[0] not in ['FIRST', 'UNITED', 'AMERICAN', 'NATIONAL', 'GENERAL', 'GLOBAL', 'INTERNATIONAL']:
                if words[0] not in self.name_to_ticker:
                    self.core_names[words[0]] = ticker
    ...
    def find_match(self, recipient_name: str) -> Optional[Dict]:
        ...
        for core_name, ticker in self.core_names.items():
            if len(core_name) >= 5 and core_name in normalized:
                # Validate it's not just a partial word match
                # Check if core_name appears as whole words
