...
class Config:
    ...
    market_hours_only: bool = False  # Changed to 24/7 mode by default
    ...
    # Debug mode
    debug_mode: bool = True  # Enable verbose logging
...
class SAMGovClient:
    ...
    def get_recent_awards(self, days_back: int = 7) -> List[Dict]:
        ...
        # Method 1: Public Opportunities API (award notices)
        try:
            ...
            self.logger.debug(f"  SAM.gov Opportunities API error: {e}")
        ...
        if errors:
            self.logger.debug(f"  SAM.gov errors: {errors}")
    ...
    def _fetch_award_notices(self, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            self.logger.debug(f"  SAM.gov API request: {url} params={params}")
            response = self.session.get(url, params=params, timeout=60)
            self.logger.debug(f"  SAM.gov API response: {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                self.logger.debug(f"SAM.gov Opportunities API returned {response.status_code}: {response.text[:200]}")
        ...
        try:
            ...
            if not awardee_name and opp.get('awardeeList'):
                ...
                if isinstance(awardee_list, list) and len(awardee_list) > 0:
                    ...
                    awardee_name = first.get('name', '') if isinstance(first, dict) else str(first)
            
            # Check award object
            if not awardee_name and opp.get('award'):
                award_obj = opp['award']
                if isinstance(award_obj, dict):
                    awardee_name = award_obj.get('awardee', '') or award_obj.get('contractor', '')
            
            # Check direct contractor field
            if not awardee_name:
                awardee_name = opp.get('contractor', '') or opp.get('contractorName', '') or opp.get('vendorName', '')
            ...
            return {
                ...
                'recipient_name': str(awardee_name).strip(),
                ...
                'date_signed': opp.get('awardDate', '') or opp.get('responseDeadLine', '') or opp.get('postedDate', ''),
...
class PublicCompanyDatabase:
    ...
    def _add_auto_variations(self, ticker: str, name: str):
        ...
        COMMON_WORDS = {
            ...
            'APPLIED', 'CENTRAL', 'NORTHERN', 'SOUTHERN', 'EASTERN', 'WESTERN',
            'PACIFIC', 'ATLANTIC', 'CONTINENTAL', 'UNIVERSAL', 'MODERN', 'CLASSIC',
            'SUMMIT', 'PINNACLE', 'FRONTIER', 'HORIZON', 'CENTURY', 'MILLENNIUM',
        }
        ...
        if len(words) >= 2:
            ...
            # Only add if neither word is a common word
            if not all(w in COMMON_WORDS for w in words[:2]):
...
class CombinedContractMonitor:
    ...
    def check_once(self) -> List[Tuple[Dict, Dict, Dict]]:
        ...
        for award in all_awards:
            ...
            if amount >= 1_000_000 and len(sample_high_value) < 10:
                sample_high_value.append(f"{recipient[:35]} -> ${amount/1e6:.1f}M")
        ...
        self.logger.info(f"  Stats: total={stats['total']}, seen={stats['seen']}, old={stats['old']}, low_value={stats['low_value']}, no_match={stats['no_match']}, low_mat={stats['low_materiality']}, no_mcap={stats['no_market_cap']}")
        ...
        if sample_high_value and stats['no_match'] > 0:
            self.logger.debug(f"  High-value unmatched: {sample_high_value[:5]}")
...
def main():
    ...
    parser.add_argument('--market-hours', action='store_true', help="Only run during market hours (9:30 AM - 4:00 PM ET)")
    ...
    config.market_hours_only = args.market_hours  # Default is False (24/7)
    config.enable_discord_alerts = not args.no_discord
    config.debug_mode = not args.no_debug  # Default is True (debug on)
