    ...
    def _fetch_award_notices(self, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            if self.config.sam_api_key:
                ...
                self.logger.info(f"  Using SAM.gov API key: {self.config.sam_api_key[:15]}...")
            else:
                self.logger.info(f"  WARNING: No SAM.gov API key configured!")
            
            self.logger.info(f"  SAM.gov API: postedFrom={params.get('postedFrom')}, postedTo={params.get('postedTo')}, ptype={params.get('ptype')}")
            response = self.session.get(url, params=params, timeout=60)
            self.logger.info(f"  SAM.gov API response: {response.status_code}")
            
            if response.status_code == 200:
                data = response.json()
                
                # Log response structure for debugging
                self.logger.info(f"  SAM.gov API response keys: {list(data.keys())}")
                self.logger.info(f"  SAM.gov API totalRecords: {data.get('totalRecords', 'N/A')}")
                ...
                self.logger.info(f"  SAM.gov API returned {len(opportunities)} opportunities")
                
                # Log first opportunity structure for debugging
                if opportunities and len(opportunities) > 0:
                    self.logger.info(f"  SAM.gov first opp keys: {list(opportunities[0].keys())[:10]}")
                ...
                for opp in opportunities:
                    ...
                    if award and award.get('recipient_name'):
                        ...
                        self.logger.info(f"  SAM.gov skipped (no recipient): {opp.get('title', 'No title')[:50]}")
            else:
                self.logger.info(f"  SAM.gov API returned {response.status_code}: {response.text[:500]}")
